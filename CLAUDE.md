...
<!-- dropin-applied: 2026-09-04(이 줄은 **현재 상태**만 — 경위는 git 히스토리) · 모드=전체 · 언어=ko · 00 v1.43 · 01 v1.66(**§B 단일 repo(B형)** — §F-1 기록 규칙 v1.66 기준 동기[최근 ~5항목·PROJECT_PLAN 대조·예정일 미완료 필터·800줄/120KB 2축·갈래+날짜·**인계 항목은 항목 앞쪽/PROJECT_PLAN**(R34)]·§F-5 `.claude/worktrees/`·산출물 언어 한 줄) · 03 v1.27(디자인 스킬은 글로벌 frontend-design·svg-design — 동명 그림자 방지로 프로젝트 설치 안 함. MCP 해당 없음[.mcp.json 없음], 브라우저 검증=Claude in Chrome) · 04 v1.21(§1 hooks 해당 없음—빌드·린트·테스트 명령이 없는 정적 HTML repo / §2 CLI는 PC 스코프 — 글로벌 기록 참조) · 미선택: 02(해당 없음—비개발 repo, 02 STEP 1 가드), 04 CI(해당 없음—PR 워크플로 미사용), 05, 06 · 출처=클론 D:\claude-dev -->
